...
export interface InvestmentsSyncResult {
    success: boolean;
    item_id: number;
    holdings: { added: number; updated: number; removed: number };
    securities: { added: number; updated: number };
...
export async function syncInvestmentsForItem(
    ...
    const result: InvestmentsSyncResult = {
        success: false,
        item_id: itemId,
    ...
    try {
        ...
        if (plaidErrorCode === 'PRODUCTS_NOT_SUPPORTED') {
            ...
            await executeQuery(
                `UPDATE items SET investments_error_code = @errorCode, updated_at = GETDATE() WHERE item_id = @itemId`,
 ...
 */
export async function syncHoldingsOnly(
    plaidClient: PlaidApi,
    itemId: number,
    accessToken: string,
    context: Context
...
async function syncHoldings(
    ...
    context: Context
): Promise<{ holdings: { added: number; updated: number; removed: number }; securities: { added: number; updated: number } }> {
    
    // Call Plaid /investments/holdings/get
    const response: InvestmentsHoldingsGetResponse = (await plaidClient.investmentsHoldingsGet({
    ...
    // 3. Process holdings
    const holdingsStats = await processHoldings(
        response.holdings || [], 
        accountMap, 
        securityMap, 
        itemId, 
...
async function processSecurities(
    securities: Security[],
    context: Context
    ...
    for (const sec of securities) {
        ...
        const existing = await executeQuery<{ security_id: number }>(
            `SELECT security_id FROM securities WHERE plaid_security_id = @plaidSecurityId`,
...
async function insertSecurity(sec: Security): Promise<void> {
...
async function updateSecurity(securityId: number, sec: Security): Promise<void> {
    // Note: Plaid SDK Security type may not have all these properties
    // We use optional chaining and type assertions for safety
    const secAny = sec as any;
...
async function processHoldings(
    ...
    const processedHoldingKeys: string[] = []; // account_id + security_id combinations
    ...
    for (const holding of holdings) {
        ...
        const existing = await executeQuery<{ holding_id: number }>(
            `SELECT holding_id FROM holdings 
             WHERE account_id = @accountId AND security_id = @securityId AND is_archived = 0`,
...
async function insertHolding(accountId: number, securityId: number, h: Holding, plaidSecurityId: string): Promise<void> {
    await executeQuery(
...
async function updateHolding(holdingId: number, h: Holding): Promise<void> {
    await executeQuery(
        `UPDATE holdings SET
...
async function archiveRemovedHoldings(
    itemId: number, 
    processedHoldingKeys: string[], 
    context: Context
): Promise<number> {
    // Get all current holdings for this item's accounts
    const currentHoldings = await executeQuery<{ holding_id: number; account_id: number; security_id: number }>(
        `SELECT h.holding_id, h.account_id, h.security_id
         FROM holdings h
         JOIN accounts a ON h.account_id = a.account_id
         WHERE a.item_id = @itemId AND h.is_archived = 0`,
        { itemId }
    );
...
async function syncInvestmentTransactions(
    ...
    const itemResult = await executeQuery<{ investment_transactions_last_synced_at: Date | null }>(
        `SELECT investment_transactions_last_synced_at FROM items WHERE item_id = @itemId`,
        { itemId }
    );
    ...
    while (hasMore) {
        const response: InvestmentsTransactionsGetResponse = (await plaidClient.investmentsTransactionsGet({
            access_token: accessToken,
        ...
        for (const txn of transactions) {
            ...
            if (txnStats === 'added') stats.added++;
            else if (txnStats === 'updated') stats.updated++;
        }
    ...
    await executeQuery(
        `UPDATE items SET investment_transactions_last_synced_at = GETDATE(), updated_at = GETDATE() WHERE item_id = @itemId`,
        { itemId }
    );
...
async function processInvestmentTransaction(
    ...
    const existing = await executeQuery<{ investment_transaction_id: number }>(
        `SELECT investment_transaction_id FROM investment_transactions 
         WHERE plaid_investment_transaction_id = @plaidTxnId`,
...
async function insertInvestmentTransaction(
    accountId: number,
...
async function getSecurityMapping(): Promise<Map<string, number>> {
    const securities = await executeQuery<SecurityMapping>(
        `SELECT security_id, plaid_security_id FROM securities`
    );
 ...
 */
export async function archiveInvestmentsForItem(itemId: number, reason: string): Promise<void> {
    // Holdings
    await executeQuery(
        `UPDATE h SET h.is_archived = 1, h.archived_at = GETDATE(), h.archive_reason = @reason
         FROM holdings h
         JOIN accounts a ON h.account_id = a.account_id
         WHERE a.item_id = @itemId AND h.is_archived = 0`,
        { itemId, reason }
    ...
    // Transactions
    await executeQuery(
        `UPDATE t SET t.is_archived = 1, t.archived_at = GETDATE(), t.archive_reason = @reason
 ...
 */
export async function archiveInvestmentsForAccounts(accountIds: number[], reason: string): Promise<void> {
    if (accountIds.length === 0) return;
    
    const idList = accountIds.join(',');

    await executeQuery(
        `UPDATE holdings SET is_archived = 1, archived_at = GETDATE(), archive_reason = @reason
         WHERE account_id IN (${idList}) AND is_archived = 0`,
        { reason }
    );

    await executeQuery(
        `UPDATE investment_transactions SET is_archived = 1, archived_at = GETDATE(), archive_reason = @reason
         WHERE account_id IN (${idList}) AND is_archived = 0`,
        { reason }
    );
}