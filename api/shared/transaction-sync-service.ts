...
import { Transaction, RemovedTransaction, TransactionsSyncResponse } from 'plaid';
...
/**
 * Item record from database with decryption info
 */
...
/**
 * Account lookup map: plaid_account_id -> internal account_id
 */
type AccountIdMap = Map<string, number>;
...
/**
 * Database transaction record for insert/update
 */
...
export async function syncTransactionsForItem(
    ...
    try {
        ...
        const syncData = await fetchAllTransactionUpdates(
            context,
            accessToken,
        ...
        const counts = await processTransactionUpdates(
            context,
            syncData.added,
            syncData.modified,
            syncData.removed,
        ...
        context.log(
            `Transaction sync complete for item ${itemId}: ` +
 ...
 */
async function getItemWithAccessToken(itemId: number): Promise<ItemRecord | null> {
    const result = await executeQuery<ItemRecord>(
        `SELECT 
            item_id,
            client_id,
            plaid_item_id,
            ...
            status
        FROM items
        WHERE item_id = @itemId AND is_archived = 0`,
        { itemId }
    );
 ...
 */
async function getAccountIdMap(itemId: number): Promise<AccountIdMap> {
    const result = await executeQuery<{ account_id: number; plaid_account_id: string }>(
    ...
    const map: AccountIdMap = new Map();
 ...
 */
async function updateItemAfterSync(itemId: number, cursor: string): Promise<void> {
    await executeQuery(
        `UPDATE items 
 ...
 */
async function fetchAllTransactionUpdates(
    context: Context,
    accessToken: string,
    ...
    while (retryCount < MAX_PAGINATION_RETRIES) {
        try {
            ...
            while (hasMore) {
                ...
                const response: TransactionsSyncResponse = await syncTransactions(accessToken, cursor);
                ...
                context.log(
                    `Page ${pageCount}: +${response.added.length} added, ` +
            ...
            if (errorMessage.includes('TRANSACTIONS_SYNC_MUTATION_DURING_PAGINATION')) {
                ...
                if (retryCount >= MAX_PAGINATION_RETRIES) {
                    throw new Error(
                        `Failed after ${MAX_PAGINATION_RETRIES} attempts due to ` +
...
async function processTransactionUpdates(
    ...
    const pendingToPostedMap = new Map<string, Transaction>();
 ...
 */
function mapPlaidTransactionToRecord(
    tx: Transaction,
    accountId: number,
 ...
 */
function detectIsTransfer(tx: Transaction): boolean {
 ...
 */
function mapConfidenceLevel(level: string | null | undefined): number | null {
    switch (level) {
        case 'VERY_HIGH':
            return 0.99;
        case 'HIGH':
...
async function upsertTransaction(
    ...
    try {
        ...
        context.log.error(
            `Failed to upsert transaction ${record.plaid_transaction_id}: ` +
...
async function markTransactionRemoved(
    ...
    try {
        ...
        context.log.error(
            `Failed to mark transaction ${plaidTransactionId} as removed: ` +
            (err instanceof Error ? err.message : String(err))
        );
 ...
 */
async function replacePendingWithPosted(
    context: Context,
    pendingPlaidId: string,
    postedTx: Transaction
): Promise<void> {
    try {
        ...
        context.log.error(
            `Failed to replace pending transaction ${pendingPlaidId}: ` +
            (err instanceof Error ? err.message : String(err))
        );
        throw err;
    }
 ...
 */
export async function getItemsWithPendingSyncUpdates(): Promise<
    Array<{ item_id: number; client_id: number; plaid_item_id: string; institution_name: string }>
> {
    const result = await executeQuery<{
        item_id: number;
        client_id: number;
        ...
        `SELECT item_id, client_id, plaid_item_id, institution_name
         FROM items
