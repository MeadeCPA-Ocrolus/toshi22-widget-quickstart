 ...
 */
async function getVerificationKey(kid: string, plaidClient: PlaidApi): Promise<JWK> {
...
export async function verifyPlaidWebhook(
    ...
    try {
        ...
        payload = result.payload as PlaidWebhookJwtPayload;
    } catch (err) {
    ...
    const actualBodyHash = createHash('sha256').update(rawBody, 'utf8').digest('hex');
