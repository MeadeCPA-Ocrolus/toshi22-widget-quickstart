...
import { 
    ...
    archiveLiabilitiesForAccounts 
} from '../shared/liabilities-sync-service';
import {
    syncInvestmentsForItem,
    syncHoldingsOnly,
    archiveInvestmentsForItem,
    archiveInvestmentsForAccounts,
} from '../shared/investments-sync-service';
    ...
    switch (webhook.webhook_code) {
        ...
        case 'USER_PERMISSION_REVOKED':
            ...
            if (itemLookup.recordset.length > 0) {
                ...
                // Archive liabilities
                try {
                    await archiveLiabilitiesForItem(internalItemId, 'item_archived');
                ...
                // Archive investments
                try {
                    await archiveInvestmentsForItem(internalItemId, 'item_archived');
...
async function handleLiabilitiesWebhook(
    ...
    const itemResult = await executeQuery<{ item_id: number; access_token: Buffer; access_token_key_id: number }>(
        `SELECT item_id, access_token, access_token_key_id 
         FROM items WHERE plaid_item_id = @plaidItemId AND is_archived = 0`,
        { plaidItemId }
    );
    ...
    switch (webhook.webhook_code) {
        case 'DEFAULT_UPDATE':
            ...
            // Sync liabilities
            const result = await syncLiabilitiesForItem(
                plaidClient,
                item.item_id,
                accessToken,
                context
 ...
 */
async function handleHoldingsWebhook(
    context: Context,
    webhook: PlaidWebhook,
    plaidClient: PlaidApi
): Promise<void> {
    ...
    switch (webhook.webhook_code) {
        case 'DEFAULT_UPDATE':
            ...
            const result = await syncHoldingsOnly(
                plaidClient,
                item.item_id,
                accessToken,
                context
 ...
 */
async function handleInvestmentTransactionsWebhook(
    context: Context,
    webhook: PlaidWebhook,
    plaidClient: PlaidApi
): Promise<void> {
    ...
    switch (webhook.webhook_code) {
        ...
        case 'HISTORICAL_UPDATE':
            ...
            const result = await syncInvestmentsForItem(
                plaidClient,
                item.item_id,
                accessToken,
                context
...
async function handleAccountRevoked(
    ...
    const accountResult = await executeQuery<{ account_id: number }>(
    ...
    if (accountResult.recordset.length > 0) {
        ...
        try {
            await archiveLiabilitiesForAccounts([accountId], 'account_revoked');
        ...
        try {
            await archiveInvestmentsForAccounts([accountId], 'account_revoked');
...
async function processSinglePublicToken(
    ...
    if (processedPlaidAccountIds.length > 0) {
        ...
        const deselectedAccounts = await executeQuery<{ account_id: number }>(
        ...
        if (deselectedAccountIds.length > 0) {
            ...
            try {
                await archiveLiabilitiesForAccounts(deselectedAccountIds, 'account_deselected');
            ...
            try {
                await archiveInvestmentsForAccounts(deselectedAccountIds, 'account_deselected');
    ...
    try {
        ...
        if (syncResult.success) {
            ...
            if (syncResult.added === 0 && syncResult.modified === 0 && syncResult.isInitialSync) {
                ...
                await executeQuery(
                    `UPDATE items SET has_sync_updates = 1, updated_at = GETDATE() WHERE item_id = @itemId`,
                    { itemId }
                );
            }
        } else {
    ...
    try {
        ...
        const plaidClient = getPlaidClient();
        const liabilitiesResult = await syncLiabilitiesForItem(plaidClient, itemId, accessToken, context);
        
        if (liabilitiesResult.success) {
            context.log(
                `${logPrefix}Initial liabilities sync complete: ` +
    ...
    try {
        ...
        const plaidClient = getPlaidClient();
        const investmentsResult = await syncInvestmentsForItem(plaidClient, itemId, accessToken, context);
        
        if (investmentsResult.success) {
            context.log(
                `${logPrefix}Initial investments sync complete: ` +
