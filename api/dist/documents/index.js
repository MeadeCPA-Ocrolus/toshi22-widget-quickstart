...
const httpTrigger = async function (context, req) {
     ...
     */
    async function confirmUpload(ctx, body, principal) {
        ...
        const blob = await (0, storage_1.verifyBlobExists)(body.blobPath);
        if (!blob.exists) {
            ctx.res = {
                status: 400,
        ...
        const existing = await (0, database_1.executeQuery)(`SELECT doc_id, filename, uploaded_at FROM documents
             WHERE client_uuid = @clientUuid AND tax_year = @taxYear AND sha256_hash = @sha256Hash`, { clientUuid: body.clientUuid, taxYear, sha256Hash: body.sha256Hash });
        ...
        const staffIdentity = (0, auth_1.getStaffIdentity)(principal);
        await (0, database_1.executeQuery)(`INSERT INTO documents (
     ...
     */
    async function listDocuments(ctx, request) {
    ...
    async function getDocument(ctx, id) {
        const result = await (0, database_1.executeQuery)(`SELECT doc_id, client_uuid, tax_year, blob_path, filename,
                    file_size, sha256_hash, uploaded_by, uploaded_at
             FROM documents
             WHERE doc_id = @docId`, { docId: id });
