 ...
 */
async function handleSingleItemSync(context, itemId) {
    ...
    const result = await (0, transaction_sync_service_1.syncTransactionsForItem)(context, itemId);
    context.res = {
 ...
 */
async function handleBulkSync(context, req) {
    ...
    let items = await (0, transaction_sync_service_1.getItemsWithPendingSyncUpdates)();
    ...
    for (const item of items) {
        const result = await (0, transaction_sync_service_1.syncTransactionsForItem)(context, item.item_id);
        results.push(result);
    }
    // Summary stats
 ...
 */
async function handleRefresh(context, itemId) {
    ...
    const result = await (0, transaction_sync_service_1.refreshTransactions)(context, itemId);
    context.res = {
        status: result.success ? 200 : 400,
        body: {
            success: result.success,
