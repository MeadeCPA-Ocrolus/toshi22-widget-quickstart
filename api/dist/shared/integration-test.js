...
async function testDatabaseConnection() {
    ...
    try {
        ...
        const result = await (0, database_1.executeQuery)('SELECT 1 AS test');
        ...
        const tables = await (0, database_1.executeQuery)(`SELECT name FROM sys.tables ORDER BY name`);
        ...
        const keys = await (0, database_1.executeQuery)(`SELECT key_id, key_name, is_active FROM encryption_keys WHERE is_active = 1`);
...
async function testEncryption() {
    ...
    try {
        ...
        const { encryptedBuffer, keyId } = await (0, encryption_1.encrypt)(testToken);
        ...
        const { encryptedBuffer: encrypted2 } = await (0, encryption_1.encrypt)(testToken);
        ...
        await (0, database_1.executeQuery)(`IF NOT EXISTS (SELECT 1 FROM clients WHERE email = 'test@integration.local')
        ...
        const clientResult = await (0, database_1.executeQuery)(`SELECT client_id FROM clients WHERE email = 'test@integration.local'`);
        ...
        await (0, database_1.executeQuery)(`INSERT INTO items (client_id, plaid_item_id, access_token, access_token_key_id, institution_id, institution_name)
        ...
        const itemResult = await (0, database_1.executeQuery)(`SELECT access_token, access_token_key_id FROM items WHERE plaid_item_id = @plaidItemId`, { plaidItemId: testPlaidItemId });
        const retrievedToken = await (0, encryption_1.decrypt)(itemResult.recordset[0].access_token, itemResult.recordset[0].access_token_key_id);
        ...
        // Cleanup test data
        await (0, database_1.executeQuery)(`DELETE FROM items WHERE plaid_item_id = @plaidItemId`, { plaidItemId: testPlaidItemId });
