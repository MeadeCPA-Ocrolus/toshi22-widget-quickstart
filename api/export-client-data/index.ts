...
import { toCsv, sanitizeFilename } from '../shared/csv-export';
...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    try {
        ...
        const clientResult = await executeQuery<ClientInfoRow & { client_id: number }>(
            `SELECT client_id, first_name, last_name, business_name, email, phone_number,
             ...
             FROM clients WHERE client_uuid = @clientUuid AND is_archived = 0`,
            { clientUuid }
        );
        ...
        const itemsResult = await executeQuery<ItemRow>(
            `SELECT item_id, institution_name, institution_id, status
             FROM items WHERE client_id = @clientId`,
            { clientId }
        );
        ...
        const clientInfoCsv = toCsv([client], [
        ...
        for (const item of itemsResult.recordset) {
            const folderName = sanitizeFilename(`${item.institution_name || 'Unknown_Bank'}_${item.item_id}`);

            // Accounts for this item
            const accountsResult = await executeQuery(
                `SELECT account_id, account_name, official_name, account_type, account_subtype, mask,
                        current_balance, available_balance, credit_limit, is_active
                 FROM accounts WHERE item_id = @itemId`,
                { itemId: item.item_id }
            );
            const accountIds = accountsResult.recordset.map((a: any) => a.account_id);

            archive.append(
                toCsv(accountsResult.recordset, [
            ...
            if (accountIds.length === 0) continue; // nothing else to export for an item with no accounts
            ...
            const transactionsResult = await executeQuery(
                `SELECT t.account_id, a.account_name, t.transaction_date, t.merchant_name, t.original_description,
            ...
            if (transactionsResult.recordset.length > 0) {
                archive.append(
                    toCsv(transactionsResult.recordset, [
                        { key: 'account_id', header: 'Account ID' },
                        { key: 'account_name', header: 'Account Name' },
            ...
            const creditResult = await executeQuery(
                `SELECT l.credit_liability_id, l.account_id, a.account_name, l.is_overdue, l.last_payment_amount, l.last_payment_date,
                 ...
                 WHERE a.item_id = @itemId AND l.is_archived = 0`,
                { itemId: item.item_id }
            );
            if (creditResult.recordset.length > 0) {
                archive.append(
                    toCsv(creditResult.recordset, [
                        { key: 'account_id', header: 'Account ID' },
                        { key: 'account_name', header: 'Account Name' },
            ...
            const creditAprsResult = await executeQuery(
                `SELECT ca.credit_liability_id, l.account_id, a.account_name, ca.apr_percentage, ca.apr_type,
            ...
            if (creditAprsResult.recordset.length > 0) {
                archive.append(
                    toCsv(creditAprsResult.recordset, [
                        { key: 'account_id', header: 'Account ID' },
                        { key: 'account_name', header: 'Account Name' },
            ...
            const studentResult = await executeQuery(
                `SELECT l.account_id, a.account_name, l.loan_name, l.origination_date, l.origination_principal_amount,
            ...
            const mortgageResult = await executeQuery(
                `SELECT l.account_id, a.account_name, l.loan_type_description, l.origination_date,
            ...
            if (mortgageResult.recordset.length > 0) {
                archive.append(
                    toCsv(mortgageResult.recordset, [
                        { key: 'account_id', header: 'Account ID' },
                        { key: 'account_name', header: 'Account Name' },
            ...
            const holdingsResult = await executeQuery(
                `SELECT h.account_id, a.account_name, s.ticker_symbol, s.name AS security_name, s.security_type,
            ...
            if (holdingsResult.recordset.length > 0) {
                archive.append(
                    toCsv(holdingsResult.recordset, [
                        { key: 'account_id', header: 'Account ID' },
                        { key: 'account_name', header: 'Account Name' },
            ...
            const invTxnResult = await executeQuery(
                `SELECT it.account_id, a.account_name, s.ticker_symbol, s.name AS security_name,
        ...
        const filename = `${sanitizeFilename(clientName)}_export.zip`;
