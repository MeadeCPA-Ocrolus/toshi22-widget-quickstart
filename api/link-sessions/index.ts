...
/**
 * Link token with session info
 */
interface LinkTokenWithStatus {
    link_token: string;  // This IS the primary key
    client_id: number;
    client_name: string;
    client_email: string;
    ...
    created_at: string;
    expires_at: string;
    used_at: string | null;  // No updated_at column, just used_at
 ...
 */
async function listLinkTokens(context: Context, req: HttpRequest): Promise<void> {
    ...
    const result = await executeQuery<LinkTokenWithStatus>(query, params);
 ...
 */
function getActionRecommendation(linkToken: LinkTokenWithStatus): {
    action: string;
    message: string;
    canResend: boolean;
} {
