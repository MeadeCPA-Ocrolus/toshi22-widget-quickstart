...
export const ClientList: React.FC = () => {
    ...
    const fetchClients = async () => {
        ...
        try {
            const response = await getClients({
                ...
                hasIssues: showOnlyIssues || undefined,
            });
            ...
            const errorMessage =
                err instanceof Error
                    ? err.message
                    : (err as { error?: string })?.error || 'Failed to load clients';
    ...
    const handleSendLink = (client: Client) => {
    ...
    const getClientStatusIndicator = (client: ClientWithAlerts) => {
        if (client.has_error) {
            return (
                <Tooltip title="Has bank connection errors">
                    <ErrorIcon color="error" sx={{ fontSize: 20 }} />
                </Tooltip>
            );
        }
        if (client.has_login_required) {
            return (
                <Tooltip title="Bank login required">
                    <Warning color="error" sx={{ fontSize: 20 }} />
                </Tooltip>
            );
        }
        if (client.has_needs_update) {
            return (
                <Tooltip title="Bank connection needs update">
                    <Warning color="warning" sx={{ fontSize: 20 }} />
                </Tooltip>
            );
        }
        if (client.has_pending_sync) {
            return (
                <Tooltip title="Transactions ready to sync">
                    <Refresh color="info" sx={{ fontSize: 20 }} />
                </Tooltip>
            );
        }
        if ((client.item_count || 0) > 0) {
            return (
                <Tooltip title="All connections healthy">
        ...
        <Box sx={{ p: 3 }}>
            ...
            {alertCounts.total > 0 && (
                <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
                    {alertCounts.loginRequired > 0 && (
                        <Card sx={{ minWidth: 140, bgcolor: 'error.light', color: 'error.contrastText' }}>
                            <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                                <Typography variant="h4" fontWeight={700}>
                                    {alertCounts.loginRequired}
                                </Typography>
                    ...
                    {alertCounts.errors > 0 && (
                        <Card sx={{ minWidth: 140, bgcolor: 'error.main', color: 'error.contrastText' }}>
                            <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                                <Typography variant="h4" fontWeight={700}>
                                    {alertCounts.errors}
                                </Typography>
                    ...
                    {alertCounts.needsUpdate > 0 && (
                        <Card sx={{ minWidth: 140, bgcolor: 'warning.light', color: 'warning.contrastText' }}>
                            <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                                <Typography variant="h4" fontWeight={700}>
                                    {alertCounts.needsUpdate}
                                </Typography>
                                <Typography variant="body2">Needs Update</Typography>
                            </CardContent>
                        </Card>
                    )}
                    {alertCounts.syncAvailable > 0 && (
                        <Card sx={{ minWidth: 140, bgcolor: 'info.light', color: 'info.contrastText' }}>
                            <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                                <Typography variant="h4" fontWeight={700}>
                                    {alertCounts.syncAvailable}
                                </Typography>
                                <Typography variant="body2">Sync Available</Typography>
                            </CardContent>
                        </Card>
                    )}
                </Box>
                        ...
                        <FormControlLabel
                            ...
                            label={
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                    ...
                                    <Typography variant="body2">Show only issues</Typography>
                                </Box>
                            }
                        />
                        <Box sx={{ flex: 1 }} />
                        <IconButton onClick={fetchClients} title="Refresh">
                            <Refresh />
                        </IconButton>
                    </Box>
                </CardContent>
                                        ...
                                        >
                                            ...
                                            <TableCell sx={{ textAlign: 'center' }}>
                                                <Chip
                                                    ...
                                                    label={client.item_count || 0}
                                                    size="small"
                                                    variant="outlined"
                                                    color={(client.item_count || 0) > 0 ? 'primary' : 'default'}
                                                />
                                            </TableCell>
                                            <TableCell sx={{ textAlign: 'center' }}>
                                                {(client.items_needing_attention || 0) > 0 ? (
                                                    <Chip
                                                        icon={<Warning sx={{ fontSize: 14 }} />}
                                                        label={client.items_needing_attention}
                                                        size="small"
                                                        color="warning"
                                                    />
                                                ) : (
            ...
            {!loading && clients.length > 0 && (
                <Paper sx={{ mt: 2, p: 1, px: 2, bgcolor: 'rgba(255, 255, 255, 0.95)' }}>
            ...
            <SendLinkDialog
                ...
                onClose={() => setSendLinkDialogOpen(false)}
